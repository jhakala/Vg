...
//      std::cout << "alt background chosen was: " << alt_bg->GetName() << std::endl;
...
//    TFile *f = new TFile("antibtag_multipdf1021.root");
//    RooWorkspace* xf = (RooWorkspace*)f->Get("wtemplates");
//    RooWorkspace *w_bg_jh2=new RooWorkspace("Vg");
//    for(int i=5; i<=5; i++){
...
//      }
//      RooAbsPdf *bg_jh2 = mpdf->getPdf(mpdf->getCurrentIndex()+i);//->clone();
//      std::cout << "background chosen was: " << bg_jh2->GetName() << std::endl;
//      w_bg_jh2->import(*bg_jh2, RooFit::RenameVariable(bg_jh2->GetName(),("bg_"+blah).c_str()));
//      w_bg_jh2->Print("V");
//      std::cout << "Step 6.1" << std::endl;
//      std::cerr<<w_bg_jh2->var("x")<<std::endl;
//      RooRealVar * range_ = w_bg_jh2->var("x");
//      std::cout << "Step 6.2" << std::endl;
//      range_->setRange(SR_lo,SR_hi);
//      char* asdf = ("bg_"+blah).c_str()	;
//      std::cout << "Step 6.3" << std::endl;
//      w_bg_jh2->import(nBackground);
//      std::cout<< "Step 7"<<std::endl;
//      std::cout<<bg_jh2->getVal() <<std::endl;
//      w_bg_jh2->pdf(asdf)->fitTo(pred, RooFit::Minimizer("Minuit2"), RooFit::Range(SR_lo, SR_hi), RooFit::SumW2Error(kTRUE), RooFit::Save());
//
//      RooArgSet* vars = w_bg_jh2->pdf(asdf)->getVariables();
...
//
//
//      bg_jh2->plotOn(aC_plot, RooFit::LineColor(i+1), RooFit::LineStyle(i+2));
//      p_1->cd();
//      aC_plot->GetYaxis()->SetRangeUser(0.01, maxdata*50.);
//      aC_plot->Draw("same");
//      TH1F *h=new TH1F();
//      h->SetLineColor(1+i);
//      h->SetLineStyle(i+2);
//      leg->AddEntry(h, bg_jh2->GetName(), "l");
//
//
//      w_bg_jh2->SaveAs((dirName+"/w_background_GaussExp.root").c_str());
//    }
//    leg->Draw();
//    //p_1->SetLogy();
//    //c_rooFit->Update();
...
//    //for (int i=0; i!=nPars[model_number]; ++i) {
//    //    std::cout<< "Step 8"<<std::endl;
//    //    std::cout<<parNames[model_number][i]<<" param "<< w_bg_jh->var(parNames[model_number][i])->getVal()<<"   "<<w_bg_jh->var(parNames[model_number][i])->getError()<<std::endl;
//    //}
//
  } else {
    std::cout << " DEBUGDEBUG " << std::endl;
    gSystem->Load("libHiggsAnalysisCombinedLimit");
    gSystem->Load("libdiphotonsUtils");
    ...
    for(int i=6; i<=6; i++){
      ...
      testJ->cd();
      //frame=x.frame();
      //RooDataHist dat("dat", "SR", RooArgList(x), h_mX_SR);
      //RooFitResult *r_bg = bg_jh->fitTo(dat, RooFit::Minimizer("Minuit2"), RooFit::Range(SR_lo, SR_hi), RooFit::SumW2Error(kTRUE), RooFit::Save());
      //bg_jh->plotOn(frame, RooFit::LineColor(kGray)); 
      //dat->plotOn(frame, RooFit::MarkerStyle(20));
      //frame->Draw();
      //h_mX_SR.Draw("SAME");
      ...
      ////RooRealVar * range_ = w_bg_jh->var("x");
      //std::cout << "Step 6.2" << std::endl;
      ////range_->setRange(SR_lo,SR_hi);
      //char* asdf = ("bg_"+blah).c_str()	;
      //std::cout << "Step 6.3" << std::endl;
      //w_bg_jh->import(nBackground);
      //std::cout<< "Step 7"<<std::endl;
      //std::cout<<bg_jh->getVal() <<std::endl;
      ////RooDataHist dat("dat", "SR", RooArgList(x), h_mX_SR);
      ////w_bg_jh->pdf(asdf)->fitTo(dat, RooFit::Minimizer("Minuit2"), RooFit::Range(SR_lo, SR_hi), RooFit::SumW2Error(kTRUE), RooFit::Save());

      //RooArgSet* vars = w_bg_jh->pdf(asdf)->getVariables();
      //TIterator *it = vars->createIterator();
      //RooRealVar* var = (RooRealVar*)it->Next();
      //while (var) {
      //  std::cout << " looking at var " << var.GetName() << std::endl;
      //  if (strcmp(var.GetName(), "x")) {
      //    var->setConstant(kTRUE);
      //    std::cout << " setting var " << var.GetName() << " to constant." << std::endl;
      //  } 
      //  var = (RooRealVar*)it->Next();
      //}
      ...
      //bg_jh->plotOn(aC_plot, RooFit::LineColor(i+1), RooFit::LineStyle(i+2));
      //p_1->cd();
      //aC_plot->GetYaxis()->SetRangeUser(0.01, maxdata*50.);
      //aC_plot->Draw("same");
      //TH1F *h=new TH1F();
      //h->SetLineColor(1+i);
      //h->SetLineStyle(i+2);
      //leg->AddEntry(h, bg_jh->GetName(), "l");


      w_bg_jh->SaveAs((dirName+"/w_background_GaussExp.root").c_str());
    }
    RooDataHist data_obs("data_obs", "Data", RooArgList(x), h_mX_SR);
    std::cout << " h_mX_SR sumOfWeights: "  << h_mX_SR->GetSumOfWeights() << std::endl;
    std::cout << " data_obs sumEntries:  "  << data_obs.sumEntries() << std::endl;
  ...
  //TH1F *h_mX_SR_fakeData=(TH1F*)h_mX_SR->Clone("h_mX_SR_fakeData");
  //h_mX_SR_fakeData->Scale(nEventsSR/h_mX_SR_fakeData->GetSumOfWeights());
  //std::cout << "Scale factor: " << nEventsSR/h_mX_SR_fakeData << std::endl;
  //RooDataHist data_obs("data_obs", "Data", RooArgList(x), h_mX_SR_fakeData);
  //RooDataHist data_obs("data_obs", "Data", RooArgList(x), h_mX_SR);
  //std::cout << " h_mX_SR: "  << h_mX_SR->GetSumOfWeights() << std::endl;
  //TCanvas* c = new TCanvas();
  //c->Draw();
  //c->cd();
  //h_mX_SR->Draw();
